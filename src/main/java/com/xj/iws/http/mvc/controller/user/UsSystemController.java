...
public class UsSystemController {
    ...
    public DataWrapper<SystemEntity> detail(
        ...
        UserEntity user = SessionManager.getSession(token);
        List<Limitation> limitations = limitationService.getLimit(user);
        DataWrapper dataWrapper = new DataWrapper<SystemEntity>();
        if (limitationService.checkSystem(user,systemId)){
            dataWrapper = systemService.detail(systemId,limitations);
        }else {
    ...
    public DataWrapper<SystemEntity> detailPack(
        ...
        if (limitationService.checkSystem(user,systemId)){
            dataWrapper =  systemService.detailPack(systemId,limitations);
        }else {
            dataWrapper.setErrorCode(ErrorCodeEnum.Limitation_error);
        }
        return dataWrapper;
